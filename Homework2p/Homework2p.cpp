...
#define INCRUCISARE_PROB 0.9
#define MUTATIE_PROB 0.009
...
void Mutate(int& n, char ch)
{
    for (int j = 0; j < n; j++)
    {
        for (int i = 0; i < L; i++) {
            ...
            bool ok = r < MUTATIE_PROB;
            if (ok) {
                ok = ok << i;
                population[j].gene[i] = population[j].gene[i] ^ ok;
...
void GenerareSirDeBiti(int sir_biti[LMAX])
{
    for (int i = 0; i < L; i++)
    {
        ...
        sir_biti[i] = nr % 2;
        sir_biti[i] = sir_biti[i] ^ (sir_biti[i] >> 1);
    }
}
...
{
    int pivot = GenerareRandomInt(1, LMAX - 2);

    int i = 0;
    for (; (i + 1) * LMAX <= pivot; i++) {
    ...
    for (int j = 0; j < LMAX; j++) {
        ...
        if (j <= dif) {
            children1.gene[i] = children1.gene[i] | (((b.gene[i] >> j) & 1) << j);

            children2.gene[i] = children2.gene[i] | (((a.gene[i] >> j) & 1) << j);
        }
        else {
            children1.gene[i] = children1.gene[i] | (((a.gene[i] >> j) & 1) << j);

            children2.gene[i] = children2.gene[i] | (((b.gene[i] >> j) & 1) << j);
        }
